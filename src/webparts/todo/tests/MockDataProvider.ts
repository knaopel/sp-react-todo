...
export default class MockDataProvider implements ITodoDataProvider {
  ...
  public createItem(title: string): Promise<ITodoItem[]> {
    const newItem: ITodoItem = this._createMockTodoItem(title, false);

    this._items[this.selectedList.Title] =
      this._items[this.selectedList.Title].concat(newItem);
  ...
  public updateItem(itemUpdated: ITodoItem): Promise<ITodoItem[]> {
    ...
    if (index !== -1) {
      this._items[this.selectedList.Title][index] = itemUpdated;
      return this.getItems();
    } else {
